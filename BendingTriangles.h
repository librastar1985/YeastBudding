...
struct CosBendingFunctor {
	...
	__device__ double operator()(const Tuuuuu &u5) {
		...
		double what_spring_constant;//, preferred_angle;
		if (SCALE_TYPE == 0){
			...
			if (edges_in_upperhem[counter]==1 || edges_in_upperhem[counter] == 0){
				what_spring_constant = (spring_constant_weak*4.0)*pow(scaling_per_edge[counter],scaling_pow) + spring_constant_weak*(1-pow(scaling_per_edge[counter],scaling_pow));
			}
			else{
				what_spring_constant = spring_constant;
			}
			...
			// if (boundaries_in_upperhem[counter] == 1){
			// 	what_spring_constant = spring_constant_weak*0.5;
			// }
			// else {}
			if (edges_in_upperhem[counter] == 1 && edges_in_tip[counter] ==1){
				what_spring_constant = spring_constant_weak;
				//preferred_angle = angle_0_bud;
			}
			else if (edges_in_upperhem[counter] == 1 && edges_in_tip[counter] != 1){
				what_spring_constant = spring_constant_weak*2.0;
				//what_spring_constant = (spring_constant_weak + spring_constant_weak)/2.0;
				//what_spring_constant = (spring_constant_weak + spring_constant)/2.0;
				//preferred_angle = (angle_0_bud + angle_0)/2.0;
			} 
			else{
				what_spring_constant = spring_constant;
				//preferred_angle = angle_0;
			}
		}
		...
		if (T1 < (INT_MAX-100) && T1 >= 0 && T2 < (INT_MAX-100) && T2 >= 0){	
			if (T1 != T2) {
	...
	//WRONG HERE!!
			...
			Mat_3x3	dcN1N2_rl = thrust::make_tuple<CVec3>(dcN1N2_rl_1, dcN1N2_rl_2, dcN1N2_rl_3);
				...
				if ((nN1 != 0.0) && (nN2 !=0.0)) {
					...
					// forceXAddr[place] = what_spring_constant * (cos(preferred_angle) * (thrust::get<0>(COSE_ri))) + what_spring_constant * (sin(preferred_angle) * SINE_rix);
					// forceYAddr[place] = what_spring_constant * (cos(preferred_angle) * (thrust::get<1>(COSE_ri))) + what_spring_constant * (sin(preferred_angle) * SINE_riy);
					// forceZAddr[place] = what_spring_constant * (cos(preferred_angle) * (thrust::get<2>(COSE_ri))) + what_spring_constant * (sin(preferred_angle) * SINE_riz);

					// idKey[place+1] = id_j;
					// forceXAddr[place+1] = what_spring_constant * (cos(preferred_angle) * (thrust::get<0>(COSE_rj))) + what_spring_constant * (sin(preferred_angle) * SINE_rjx);
					// forceYAddr[place+1] = what_spring_constant * (cos(preferred_angle) * (thrust::get<1>(COSE_rj))) + what_spring_constant * (sin(preferred_angle) * SINE_rjy);
					// forceZAddr[place+1] = what_spring_constant * (cos(preferred_angle) * (thrust::get<2>(COSE_rj))) + what_spring_constant * (sin(preferred_angle) * SINE_rjz);

					// idKey[place+2] = id_k;
					// forceXAddr[place+2] = what_spring_constant * (cos(preferred_angle) * (thrust::get<0>(COSE_rk))) + what_spring_constant * (sin(preferred_angle) * SINE_rkx);
					// forceYAddr[place+2] = what_spring_constant * (cos(preferred_angle) * (thrust::get<1>(COSE_rk))) + what_spring_constant * (sin(preferred_angle) * SINE_rky);
					// forceZAddr[place+2] = what_spring_constant * (cos(preferred_angle) * (thrust::get<2>(COSE_rk))) + what_spring_constant * (sin(preferred_angle) * SINE_rkz);

					// idKey[place+3] = id_l;
					// forceXAddr[place+3] = what_spring_constant * (cos(preferred_angle) * (thrust::get<0>(COSE_rl))) + what_spring_constant * (sin(preferred_angle) * SINE_rlx);
					// forceYAddr[place+3] = what_spring_constant * (cos(preferred_angle) * (thrust::get<1>(COSE_rl))) + what_spring_constant * (sin(preferred_angle) * SINE_rly);
					// forceZAddr[place+3] = what_spring_constant * (cos(preferred_angle) * (thrust::get<2>(COSE_rl))) + what_spring_constant * (sin(preferred_angle) * SINE_rlz);
				...
				//energy = spring_constant * (1 - cos(theta_current - preferred_angle) );
